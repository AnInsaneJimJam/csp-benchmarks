...
fn build_guest_bins() {
    ...
    let mut args = vec!["build", "--bins", "--target-dir", "target"];
    if is_release {
    ...
    if !output.status.success() {
        ...
        panic!("cargo build of Ceno guest binaries failed.");
    }

    for example in glob::glob("guest/src/bin/*.rs")
        .unwrap()
        .map(Result::unwrap)
    {
        ...
        writeln!(
            ...
            pub const {example}: &[u8] =
                include_bytes!(r"{CARGO_MANIFEST_DIR}/guest/target/riscv32im-ceno-zkvm-elf/{}/{example}");"#,
            std::env::var("PROFILE").unwrap()
        )
        .expect("failed to write vars.rs");
    }

    rerun_all_but_target(Path::new("guest"));
}
